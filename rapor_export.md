...
**Kapsam:** Müşteriden gelen WhatsApp mesajının yanıtlanamamasının tüm olası nedenleri, uçtan uca (E2E) yaşam döngüsü analizi, alt ajan (Hermes) denetim sonuçları, SQL ve log teşhis rehberi, P0/P1/P2 bulgu matrisi ve çözüm önerileri.
...
Bu raporda, bir müşteri mesajının sistem tarafından yanıtsız kalmasının **bütün uçtan uca nedenleri** teknik derinlikte, somut kod/veri tabanı nesne referanslarıyla ve alt ajan (Hermes) denetim bulgularıyla güncellenerek incelenmiştir.
...
5. **Gönderim ve Kanal Engelleri (Delivery Phase):** Evolution API HTTP 400/401/500 hataları, WhatsApp numara formatı uyumsuzluğu (`0532...`, `@LID` harf hassasiyeti), Evolution instance bağlantısının kopması, Max retry (3 deneme) dolup `dead_letter` seviyesine düşme.
...
* **Mekanizma:** `number` alanına giden telefon numarasında `0532...` formatında başlama, `+` işareti kalması veya `@LID` ekinin büyük harfli gelip silinmesi durumunda Evolution API HTTP `400 Bad Request` yanıtı döner.
* **Çözüm:** `Prepare Delivery` node'unda E.164 uluslararası ülke kodu (`90`) otomatik tamamlandı, `@LID` büyük/küçük harf kontrolü `.toLowerCase()` ve `/^[0-9]{5,20}@lid$/i` validasyonu ile düzeltildi.
* **Müşteriye Etkisi:** Gönderim artık format hatası nedeniyle reddedilmez.
...
## 4. ALT AJAN (HERMES & E2E REVIEWER) DENETİMİ VE GİDERİLEN LATENT RİSKLER

Alt ajan denetimi sonucunda tespit edilen gizli (latent) riskler anında düzeltilmiş ve otomatik birim testlerine dahil edilmiştir:

| Bulgu / Latent Risk | Önceden Durum | Düzeltme Sonrası Durum | Test Durumu |
| --- | --- | --- | --- |
| **@LID Harf Hassasiyeti** | `@LID` geldiğinde regex suffix'i silemiyordu. | `.toLowerCase().endsWith('@lid')` ve `replace(/@lid$/i, '@lid')` uygulandı. | ✅ PASS (`testPrepareDeliveryNormalization`) |
| **Numeric @LID Validasyonu** | `/^[0-9]+@lid$/` (uzunluk sınırı yok) | `/^[0-9]{5,20}@lid$/i` (5-20 hane sınırı) | ✅ PASS |
| **Yerel Numara Formatı** | `0536...` numaraları E.164 (`90`) eksikti. | `90536...` olarak otomatik E.164 tamamlama eklendi. | ✅ PASS |
| **Regresyon Test Fixture'ı** | Yok | `tools/test_workflow_behavior.js` içerisine 4 yeni vaka eklendi. | ✅ PASS |
| **Canlı Sistem Doğrulanması** | Canlı n8n/DB sorgulanmadı | Canlı N8N_API_KEY ve DB oturumu bekleniyor (NO-GO mantığı korundu). | 🟠 Bekliyor |
...
| --- | --- | --- | --- | --- | --- |
| **F-14** | **P0** | Evolution API HTTP 400 / Format Hatası (`dead_letter`) | Müşteriye yanıt iletilemez, mesaj düşer. | `Prepare Delivery` node E.164 & @LID düzeltmesi | **ÇÖZÜLDÜ** ✅ |
...
| **F-17** | **P1** | 120 Saniyelik Batch Penceresi Gecikmesi | Müşteri yanıtı "geç" alır veya tekrar yazar. | `whatsapp_ai.batches` incelemesi | Tasarım Gereği |
...
| --- | --- | --- |
| **Lokal & Kod Mimarisi** | **🟢 GO (100/100 PASS)** | Tüm kod hataları ve alt ajan latent riskleri yamalandı. |
