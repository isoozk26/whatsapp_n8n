-- 067: Notify administrators when authorized manual-mode commands succeed.
-- Keeps the live 8-argument ingest_message signature and is idempotent.
...
BEGIN
    ...
    ELSIF p_command = 'check_mode' THEN
        ...
        v_msg_text := E'📊 SİSTEM DURUMU\n👤 Müşteri: ' || p_sender_name || E'\n' || CASE WHEN EXISTS (SELECT 1 FROM whatsapp_ai.manual_modes WHERE sender_number = p_sender_number AND enabled) THEN E'🤖 Mod: MANUEL\n⚡ Durum: Manuel mod aktif' ELSE E'🤖 Mod: OTOMATİK\n⚡ Durum: Aktif' END;
        IF v_admin_phone_a <> '' AND v_admin_phone_a <> p_sender_number THEN
