-- 040_delivery_priority.sql
-- Adds delivery priority levels so customer replies are sent before admin notifications.

BEGIN;

-- 1. Add priority column with default 50 (existing rows get standard priority)
ALTER TABLE whatsapp_ai.deliveries
    ADD COLUMN IF NOT EXISTS priority integer NOT NULL DEFAULT 50;

-- 2. Backfill existing deliveries based on channel
...
-- 3. Replace index to include priority for efficient ORDER BY priority DESC, created_at ASC
...
-- 4. Replace claim_deliveries to ORDER BY priority DESC, created_at ASC
...
-- 5. Replace complete_ai_batch to set priority on delivery insert
...
BEGIN
    ...
    -- Admin notification priority: 90 for handoff alerts, 50 for regular admin notifications
    IF p_notify_admins AND p_admin_phone_a <> '' AND NOT EXISTS (
        ...
        VALUES (gen_random_uuid(), p_batch_token, p_sender_number, 'phone_a', p_admin_phone_a,
                ...
                CASE WHEN p_pause_automation THEN 90 ELSE 50 END)
...
-- 6. Replace record_ai_failure to set priority 40 for command/AI-failure admin notifications
