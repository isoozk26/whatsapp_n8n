...
BEGIN
    ...
    IF p_case_type = 'unclear' THEN
        ...
        SET count = CASE
                WHEN whatsapp_ai.unclear_counts.expires_at < clock_timestamp() THEN 1
                ELSE whatsapp_ai.unclear_counts.count + 1
            END,
    ...
    IF p_notify_admins AND p_admin_phone_a <> '' AND NOT EXISTS (
        SELECT 1
        FROM whatsapp_ai.admin_notifications
        WHERE sender_number = p_sender_number
          AND channel = 'phone_a'
          AND fingerprint = p_fingerprint
          AND sent_at > clock_timestamp() - interval '3 minutes'
    ) THEN
    ...
    SET status = 'pending',
        ...
        first_message_at = CASE
            WHEN jsonb_array_length(v_pending) > 0 THEN clock_timestamp()
            ELSE NULL
        END,
...
BEGIN
    ...
    IF v_attempt >= 4 THEN
        ...
        IF p_admin_phone_a <> '' THEN
            INSERT INTO whatsapp_ai.deliveries(id, batch_token, sender_number, channel, destination, payload)
            VALUES (gen_random_uuid(), p_batch_token, p_sender_number, 'phone_a', p_admin_phone_a, jsonb_build_object('number', p_admin_phone_a, 'text', 'AI islemi 4 kez basarisiz oldu. Musteri manuel incelemeye alindi: ' || p_sender_number, 'correlationId', p_correlation_id)) ON CONFLICT (batch_token, channel) DO NOTHING;
        END IF;

        IF p_admin_phone_b <> '' THEN
            INSERT INTO whatsapp_ai.deliveries(id, batch_token, sender_number, channel, destination, payload)
            VALUES (gen_random_uuid(), p_batch_token, p_sender_number, 'phone_b', p_admin_phone_b, jsonb_build_object('number', p_admin_phone_b, 'text', 'AI islemi 4 kez basarisiz oldu. Musteri manuel incelemeye alindi: ' || p_sender_number, 'correlationId', p_correlation_id)) ON CONFLICT (batch_token, channel) DO NOTHING;
        END IF;

        INSERT INTO whatsapp_ai.system_events(event_type, sender_number_masked, batch_token, details)
        VALUES ('ai_dead', right(p_sender_number, 4), p_batch_token, jsonb_build_object('code', p_error_code, 'attempt', v_attempt));
