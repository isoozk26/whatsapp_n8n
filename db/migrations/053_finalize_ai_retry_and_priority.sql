...
DROP FUNCTION IF EXISTS whatsapp_ai.complete_ai_batch(text, uuid, text, text, boolean, boolean, boolean, text, text);
DROP FUNCTION IF EXISTS whatsapp_ai.record_ai_failure(text, uuid, text, text);
...
BEGIN
    ...
    ELSIF p_case_type = 'non_product' THEN
        v_admin_priority := CASE
            WHEN upper(p_admin_message) LIKE '%ŞİKAYET%' THEN 95
            WHEN upper(p_admin_message) LIKE '%İADE%' THEN 90
            WHEN upper(p_admin_message) LIKE '%TEMSİLCİ%' THEN 85
            ELSE 50
        END;
    ELSE
        v_admin_priority := 50;
    ...
    IF p_notify_admins AND p_admin_phone_a <> '' AND NOT EXISTS (
        ...
        VALUES (
            ...
            p_admin_phone_a,
            jsonb_build_object(
                'number', p_admin_phone_a,
                'text', p_admin_message,
    ...
    IF p_reply_customer AND p_customer_reply <> '' THEN
        ...
        VALUES (
            ...
            p_sender_number,
            jsonb_build_object(
                'number', p_sender_number,
                'text', p_customer_reply,
                'correlationId', p_correlation_id,
                '_deliveryType', 'customer_reply'
            ),
            100
        )
        ON CONFLICT (batch_token, channel) DO NOTHING;
    ...
    IF p_pause_automation THEN
        ...
        VALUES (
            ...
            p_batch_token,
            jsonb_build_object('pauseAutomation', true, 'caseType', p_case_type, 'correlationId', p_correlation_id)
        );
    END IF;
    ...
    IF v_attempt >= 3 THEN
        ...
        IF p_admin_phone_a <> '' THEN
            ...
            VALUES (
                ...
                jsonb_build_object(
                    'number', p_admin_phone_a,
                    'text', 'AI islemi 3 kez basarisiz oldu. Musteri manuel incelemeye alindi: ' || p_sender_number,
                    'correlationId', p_correlation_id,
        ...
        VALUES (
            ...
            right(p_sender_number, 4),
            p_batch_token,
            jsonb_build_object('code', p_error_code, 'attempt', v_attempt, 'correlationId', p_correlation_id)
        );
        RETURN 'manual';
    END IF;
