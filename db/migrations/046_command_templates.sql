...
BEGIN
    ...
    IF p_command = 'pause' THEN
        ...
        IF EXISTS (
            ...
            VALUES (
                gen_random_uuid(), gen_random_uuid(), p_sender_number, 'phone_a', v_admin_phone_a,
                jsonb_build_object('number', v_admin_phone_a, 'text',
                    E'\U0001f512 SİSTEM MANUEL MODA GEÇTİ\n\U0001f464 Müşteri: ' || p_sender_name || ' (' || p_sender_number || E')\n\u26a1 Otomatik AI yanıtları durduruldu\n\U0001f4ac Temsilci yanıtı bekleniyor\n\U0001f4a1 İpucu: Otomatiğe almak için "--" yazın',
                    'fingerprint', 'cmd:pause:' || p_sender_number || ':' || extract(epoch FROM clock_timestamp())::text
                )
    ...
    ELSIF p_command = 'check_mode' THEN
        ...
        IF v_mode_status THEN
            ...
            RETURN QUERY SELECT 'command'::text, 0, 'Müşteri zaten manuel modda. Değişiklik yapılmadı.'::text;
            RETURN;
        ELSE
            ...
            RETURN QUERY SELECT 'command'::text, 0, 'Müşteri zaten otomatik modda. Değişiklik yapılmadı.'::text;
            RETURN;
        END IF;
    END IF;
