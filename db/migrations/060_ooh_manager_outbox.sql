...
CREATE TABLE IF NOT EXISTS whatsapp_ai.ooh_manager_dispatch (
    ooh_log_id uuid NOT NULL REFERENCES whatsapp_ai.ooh_log(id) ON DELETE CASCADE,
    channel text NOT NULL CHECK (channel IN ('phone_a', 'phone_b')),
    delivery_id uuid NOT NULL,
    queued_at timestamptz NOT NULL DEFAULT clock_timestamp(),
    PRIMARY KEY (ooh_log_id, channel),
    UNIQUE (delivery_id)
);
