...
parse_ai_output_js = (
    ...
    "  const codesText = (entities.productCodes && entities.productCodes.length > 0) ? entities.productCodes.join(', ') : 'İlettiğiniz ürün(ler)';\n"
    "  replyDraft = `${codesText} talebiniz alındı. Stok ve net fiyat bilgisini birkaç dakika içinde sizinle paylaşıyoruz.`;\n"
    "  if (!entities.quantity || entities.quantity === 'belirtilmedi') {\n"
    "    replyDraft += ' Fiyatlandırma için kaç adet istediğinizi paylaşabilir misiniz?';\n"
    "  }\n"
    ...
    "  `Atanan kişi/departman: Stok Sorumlusu — SLA: 5 dk\\n` +\n"
    "  `Stok yoksa alternatif: (Yetkili tarafından belirlenecek)\\n` +\n"
