...
store_context_js = (
    ...
    "const isWebhookRequest = Boolean(firstInput?.body?.data?.key?.remoteJid);\n\n"
    "if (isWebhookRequest && expectedSecret) {\n"
    "    const queryToken = firstInput?.query?.token || '';\n"
    "    const headerToken = firstInput?.headers?.['x-webhook-secret'] || '';\n"
    "    const bodySecret = firstInput?.body?.secret || '';\n"
...
parse_ai_output_js = (
    ...
    "}\n\n"
    "if (isUpdate) {\n"
    "  headerTitle = '🔥 SATIŞ GÖREVİ (GÜNCELLENDİ)';\n"
    "}\n\n"
    ...
    "}\n\n"
    "let headerEmoji = '📌';\n"
    "if (caseType === 'exact_code_price_stock' || currentCodes.length > 0) {\n"
    "  headerEmoji = '🔥';\n"
    "} else if (caseType === 'non_product' || action === 'handoff') {\n"
    "  headerEmoji = '⚠️';\n"
    "} else if (caseType === 'exact_code_compatibility' || intent === 'product_compatibility' || caseType === 'vehicle_based_search') {\n"
    "  headerEmoji = '🚗';\n"
    "} else if (caseType === 'cross_reference') {\n"
    "  headerEmoji = '🔄';\n"
    "}\n"
    "\n"
    "const cleanTitle = headerTitle.replace(/^[🔥⚠️🚗📌🔄❓?]+\\s*/, '');\n"
    "const cleanProducts = currentCodes.length > 0 ? currentCodes.join(', ') : (provenanceViolation ? `Şüpheli Kod: ${unverifiedCode}` : 'Belirtilmedi');\n"
    "const cleanVehicles = Array.isArray(entities.vehicles) && entities.vehicles.length > 0 ? entities.vehicles.map(v => typeof v === 'object' ? `${v.brand || ''} ${v.model || ''} ${v.year || ''}`.trim() : `${v}`).join(', ') : 'Belirtilmedi';\n"
    "\n"
    "const yapilacaklarList = yapilacaklar.split('\\n').map(l => l.replace(/^\\d+\\.\\s*/, '✓ ')).join('\\n');\n"
    "\n"
    "const bildirim = `${headerEmoji} ${cleanTitle}\\n` +\n"
    "  `⏱ SLA: 5 dk\\n\\n` +\n"
    "  `👤 ${senderName || 'Bilinmeyen Müşteri'}\\n` +\n"
    "  `📞 ${senderNumber || 'Bilinmiyor'}\\n\\n` +\n"
    "  `📦 Ürün\\n${cleanProducts}\\n\\n` +\n"
    "  `🚗 Araç\\n${cleanVehicles}\\n\\n` +\n"
    "  `🎯 İstenen\\n${yapilacaklarList}\\n\\n` +\n"
    "  `👨 Atanan\\nİsmail Özkaracan` +\n"
