...
parse_ai_output_js = (
    ...
    "    let str = typeof v === 'object' ? `${v.brand||\'\'} ${v.model||\'\'} ${v.year||\'\'} ${v.engine||\'\'} ${v.raw||\'\'}` : String(v);\n"
    ...
    "    if (words.length > 0 && !words.some(w => textLower.includes(w))) return null;\n"
    "    return v;\n"
    "  }).filter(Boolean);\n"
    "}\n\n"
    ...
    "  replyDraft = `${codesText} için ${miktarText}talebiniz alındı. Güncel stok ve net fiyat Satış Temsilcimiz İsmail Bey tarafından kontrol edilerek en geç 5 dakika içinde size iletilecektir.`;\n"
