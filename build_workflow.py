...
batch_collector_js = (
    ...
    "const input = $json;\n\n"
    "const payload = input.body?.body?.data ? input.body.body.data : input.body.data;\n"
    "const rawJid = payload?.key?.remoteJid || '';\n"
    "const senderNumber = rawJid.replace('@s.whatsapp.net', '').replace('@g.us', '').replace('@lid', '');\n"
    "const senderName = payload?.pushName || senderNumber;\n"
    ...
    "    commandParticipant: payload?.key?.participant || '',\n"
    ...
    "  }}];\n"
    "}\n\n"
    "const _manualMode = staticData._manualModes[senderNumber];\n"
    "if (fromMe || (_manualMode && _manualMode.enabled === true)) {\n"
    "  const MANUAL_TTL_MS = 30 * 60 * 1000;\n"
    "  if (_manualMode && _manualMode.enabled === true && _manualMode.createdAt) {\n"
    "    if (Date.now() - Number(_manualMode.createdAt) > MANUAL_TTL_MS) {\n"
    "      delete staticData._manualModes[senderNumber];\n"
    "    } else {\n"
    ...
    "}\n\n"
    "const hasImage = payload?.message?.imageMessage ? true : false;\n"
    ...
    "  mediaUrl: payload?.message?.imageMessage?.url || null,\n"
    "  mediaKey: payload?.message?.imageMessage?.mediaKey || null,\n"
...
stale_batch_check_js = (
    ...
    "if (!staticData._batches) staticData._batches = {};\n"
    "if (!staticData._manualModes) staticData._manualModes = {};\n"
    "for (const [number, manualEntry] of Object.entries(staticData._manualModes)) {\n"
    "  if (!manualEntry || typeof manualEntry !== 'object' || !manualEntry.enabled) {\n"
    ...
    "const _me = staticData._manualModes[number];\n"
    "  if (_me && _me.enabled === true) {\n"
    "    const MANUAL_TTL_MS = 30 * 60 * 1000;\n"
    "    if (_me.createdAt && (Date.now() - Number(_me.createdAt)) > MANUAL_TTL_MS) {\n"
    "      delete staticData._manualModes[number];\n"
    "    } else {\n"
...
parse_ai_output_js = (
    ...
    "  const complaintKeywords = ['şikayet', 'iade', 'geri', 'para iadesi', 'bozuk', 'hasarlı', 'kırık', 'yanlış', 'eksik', 'sorunlu', 'memnun değil', 'bayilik', 'temsilci', 'kargo', 'gelmedi', 'ulaşmadı', 'sipariş', 'ödeme', 'çekildi', 'iptal', 'değişim', 'yetkili', 'canlı destek', 'insan', 'görüşmek'];\n"
    ...
    "if (shouldNotifyAdmin) { expectedChannels.phoneA = true; expectedChannels.phoneB = true; }\n"
    ...
    "  .replace(/MÃ¼ÅŸteri/g, 'Musteri')\n"
    "  .replace(/AraÃ§/g, 'Arac')\n"
    "  .replace(/Ä°stenen/g, 'Istenen')\n"
    "  .replace(/Ä°smail Ã–zkaracan/g, 'Ismail Ozkaracan')\n"
    "  .replace(/âœ“/g, '-')\n"
    "  .replace(/ğŸ‘¤/g, 'Musteri:')\n"
    "  .replace(/ğŸ“/g, 'Telefon:')\n"
    "  .replace(/ğŸ“¦/g, '')\n"
    "  .replace(/ğŸš—/g, '')\n"
    "  .replace(/ğŸ¯/g, '')\n"
    "  .replace(/ğŸ‘¨/g, '')\n"
    ...
    "  .replace(/â±/g, 'SLA:')\n"
    "  .replace(/â”€+/g, '--------------');\n\n"
    "let headerTitleClean = '📢 MÜŞTERİ TALEBİ / BİLDİRİM';\n"
    "if (caseType === 'non_product' || action === 'handoff') headerTitleClean = '⚠️ ŞİKAYET / İADE / TEMSİLCİ TALEBİ';\n"
    "else if (caseType === 'exact_code_compatibility' || intent === 'product_compatibility') headerTitleClean = '🛠️ UYUMLULUK VE PARÇA KONTROLÜ';\n"
    "else if (caseType === 'cross_reference') headerTitleClean = '🔄 MUADİL / ÇAPRAZ REFERANS TALEBİ';\n"
    "else if (caseType === 'exact_code_price_stock' || currentCodes.length > 0) headerTitleClean = '🔥 YÜKSEK NİYETLİ SATIŞ TALEBİ';\n"
    "else if (caseType === 'vehicle_based_search') headerTitleClean = '🚗 ARAÇ BAZLI PARÇA ARAMA';\n"
    "if (isUpdate) headerTitleClean = '🔄 SATIŞ TALEBİ GÜNCELLENDİ';\n"
    "const cleanCodes = String(codeStr || 'Belirtilmedi').replace(/â€¢/g, '•').replace(/Ãœ/g, 'Ü').replace(/Å/g, 'Ş').replace(/Ä°/g, 'İ');\n"
    "const cleanVehicles = String(formattedVehicles || 'Belirtilmedi').replace(/â€¢/g, '•').replace(/AraÃ§/g, 'Araç').replace(/deÄŸil/g, 'değil');\n"
    "const bildirimFinal = `${headerTitleClean}\n"
    "⏱️ SLA: 5 dk\n\n"
    "👤 ${senderName}\n"
    "📞 ${senderNumber}\n\n"
    "📦 Ürün/Kod\n${cleanCodes}\n\n"
...
clear_batch_js = (
    ...
    "  if (!completedChannel && input.validClaim === true) {\n"
    ...
    "staticData._finalizedTokens = Object.assign({}, staticData._finalizedTokens);\n\n"
    "if (!staticData._manualModes) staticData._manualModes = {};\n\n"
...
connections = {
    ...
    "AI Agent": {"main": [[{"node": "Parse AI Output", "type": "main", "index": 0}]]},
      "Parse AI Output": {"main": [[{"node": "Should Notify Admins?", "type": "main", "index": 0}, {"node": "Should Reply Customer?", "type": "main", "index": 0}, {"node": "Finalize Batch", "type": "main", "index": 0}]]},
      "Finalize Batch": {"main": [[]]},
